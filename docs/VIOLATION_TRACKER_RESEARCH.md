...
**Branch:** `feature/violation-tracker-integration`  
...
### Recommended Phase 2: optional subscriber/export mode

If the firm later purchases a subscription or receives an authorized dataset/export, add a second ingestion path that loads the downloaded CSV/XML into a local cache and performs all bidder matching locally.

This would be the more stable and efficient method for larger recurring workloads because it avoids repeatedly querying the public website. It should be treated as an optional provider behind the same source adapter contract so the comparison logic does not care whether evidence came from targeted public pages or an authorized export.
...
When implementation begins, add a dedicated adapter under `backend/app/research/sources/violation_tracker.py` using the existing `ResearchSource` contract.
...
**Better long-term option:** if the firm obtains authorized CSV/XML/full-dataset access, switch acquisition to local dataset ingestion while preserving the same matching, evidence, and comparison logic.
